...
TIMESTAMPS_FILE_PATH = r'C:\Users\noobd\Desktop\TAJWEED AI\download_script\extracted_timestamps\husary_by_surah\113_Al-Falaq_timestamps.json'
AUDIO_BASE_PATH = r'C:\Users\noobd\Desktop\TAJWEED AI\download_script\downloaded_quran_audio_direct\113_Al-Falaq'
OUTPUT_FILE = 'falaq_tajweed_annotations_local.json'
...
def load_timestamps():
    """Load the local timestamps file."""
    try:
        with open(TIMESTAMPS_FILE_PATH, 'r', encoding='utf-8') as f:
            return json.load(f)
    except FileNotFoundError:
        print(f"Error: Timestamps file not found at '{TIMESTAMPS_FILE_PATH}'")
        return None
    except json.JSONDecodeError:
        print(f"Error: Could not decode JSON from '{TIMESTAMPS_FILE_PATH}'")
        return None
    except Exception as e:
        print(f"An unexpected error occurred: {e}")
        return None

def get_local_audio_path(ayah_number):
    """Get the local path to the audio file for a given ayah."""
...
def create_falaq_annotations_local():
    """
    Create annotated dataset for Surah Al-Falaq with local audio files and timestamps.
    """
    # Load timestamps
    ...
    annotations = {
        "metadata": {
            ...
            "description": "Tajweed annotation dataset for Surah Al-Falaq with local audio files and timestamps",
            "audio_source": "Local Husary recordings",
    ...
    for ayah_num in range(1, 6):
        ...
        local_audio_path = get_local_audio_path(ayah_num)
        ...
        ayah_annotation = {
            ...
            "reciter": ayah_timestamps.get("reciter", "Husary"),
...
def print_summary(annotations):
    """
    Print a summary of the local Qalqalah detection results.
    """
    if not annotations:
        return
    
    print("\n" + "="*70)
    print("SURAH AL-FALAQ LOCAL AUDIO QALQALAH DETECTION SUMMARY")
    print("="*70)
    ...
    print(f"\n🎵 AUDIO SOURCE: Local Husary recordings")
    print(f"📊 TIMESTAMPS: Extracted from Tarteel API")
    ...
    for ayah_num in range(1, 6):
        ...
        print(f"Audio: {ayah['local_audio_path']}")
        print(f"Reciter: {ayah['reciter']}")
    ...
    print(f"\n💾 LOCAL FILES:")
    print(f"   - Audio files: {AUDIO_BASE_PATH}")
    print(f"   - Timestamps: {TIMESTAMPS_FILE_PATH}")
    print("="*70)

def main():
    """
    Main function to run the local Qalqalah detection for Surah Al-Falaq.
    """
    print("Analyzing Surah Al-Falaq for strict Qalqalah rules with local audio...")
    
    # Create annotations
    annotations = create_falaq_annotations_local()
    ...
    if save_annotations(annotations, OUTPUT_FILE):
        print(f"Successfully created local annotated dataset for Surah Al-Falaq")
